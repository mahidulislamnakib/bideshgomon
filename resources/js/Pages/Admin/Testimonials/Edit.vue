<template>
    <AdminLayout title="Edit Testimonial">
        <div class="max-w-4xl mx-auto">
            <div class="mb-6">
                <Link :href="route('admin.testimonials.index')" class="inline-flex items-center text-gray-600 hover:text-gray-900 dark:text-white">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
                    </svg>
                    Back to Testimonials
                </Link>
            </div>

            <div class="bg-white rounded-2xl shadow-sm p-6">
                <h2 class="text-2xl font-bold text-gray-800 mb-6">Edit Testimonial</h2>

                <form @submit.prevent="submit" class="space-y-6">
                    <!-- Author Name -->
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">
                                Author Name (English) *
                            </label>
                            <input
                                v-model="form.author_name"
                                type="text"
                                required
                                class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-growth-600 focus:border-growth-600 transition-all resize-none"
                                :class="{ 'border-red-500': form.errors.author_name }"
                            />
                            <div v-if="form.errors.author_name" class="mt-1 text-sm text-red-600">
                                {{ form.errors.author_name }}
                            </div>
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">
                                Author Name (Bengali)
                            </label>
                            <input
                                v-model="form.author_name_bn"
                                type="text"
                                class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-growth-600 focus:border-growth-600 transition-all resize-none"
                            />
                        </div>
                    </div>

                    <!-- Position -->
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">
                                Position (English)
                            </label>
                            <input
                                v-model="form.position"
                                type="text"
                                class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-growth-600 focus:border-growth-600 transition-all resize-none"
                            />
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">
                                Position (Bengali)
                            </label>
                            <input
                                v-model="form.position_bn"
                                type="text"
                                class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-growth-600 focus:border-growth-600 transition-all resize-none"
                            />
                        </div>
                    </div>

                    <!-- Company -->
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">
                                Company (English)
                            </label>
                            <input
                                v-model="form.company"
                                type="text"
                                class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-growth-600 focus:border-growth-600 transition-all resize-none"
                            />
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">
                                Company (Bengali)
                            </label>
                            <input
                                v-model="form.company_bn"
                                type="text"
                                class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-growth-600 focus:border-growth-600 transition-all resize-none"
                            />
                        </div>
                    </div>

                    <!-- Location -->
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">
                            Location
                        </label>
                        <input
                            v-model="form.location"
                            type="text"
                            placeholder="e.g., Dhaka, Bangladesh"
                            class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-growth-600 focus:border-growth-600 transition-all resize-none"
                        />
                    </div>

                    <!-- Content -->
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">
                                Testimonial Content (English) *
                            </label>
                            <textarea
                                v-model="form.content"
                                required
                                rows="6"
                                class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-growth-600 focus:border-growth-600 transition-all resize-none"
                                :class="{ 'border-red-500': form.errors.content }"
                            />
                            <div v-if="form.errors.content" class="mt-1 text-sm text-red-600">
                                {{ form.errors.content }}
                            </div>
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">
                                Testimonial Content (Bengali)
                            </label>
                            <textarea
                                v-model="form.content_bn"
                                rows="6"
                                class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-growth-600 focus:border-growth-600 transition-all resize-none"
                            />
                        </div>
                    </div>

                    <!-- Rating -->
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">
                            Rating *
                        </label>
                        <div class="flex items-center gap-2">
                            <button
                                v-for="star in 5"
                                :key="star"
                                type="button"
                                @click="form.rating = star"
                                class="focus:outline-none"
                            >
                                <svg
                                    class="h-8 w-8 transition-colors"
                                    :class="star <= form.rating ? 'text-yellow-400' : 'text-gray-300'"
                                    fill="currentColor"
                                    viewBox="0 0 20 20"
                                >
                                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                </svg>
                            </button>
                            <span class="ml-2 text-sm text-gray-600">{{ form.rating }} star{{ form.rating !== 1 ? 's' : '' }}</span>
                        </div>
                        <div v-if="form.errors.rating" class="mt-1 text-sm text-red-600">
                            {{ form.errors.rating }}
                        </div>
                    </div>

                    <!-- Photo Upload -->
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">
                            Author Photo
                        </label>
                        
                        <div v-if="testimonial.photo && !imagePreview" class="mb-3">
                            <p class="text-sm text-gray-600 mb-2">Current Photo:</p>
                            <img :src="`/storage/${testimonial.photo}`" alt="Current photo" class="w-24 h-24 rounded-full object-cover"/>
                        </div>
                        
                        <input
                            @change="handleFileChange"
                            type="file"
                            accept="image/*"
                            class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-growth-600 focus:border-growth-600 transition-all resize-none"
                        />
                        <p class="mt-1 text-sm text-gray-500">Maximum file size: 2MB. Leave empty to keep current photo.</p>
                        
                        <div v-if="imagePreview" class="mt-3">
                            <p class="text-sm text-gray-600 mb-2">New Photo Preview:</p>
                            <img :src="imagePreview" alt="Preview" class="w-24 h-24 rounded-full object-cover"/>
                        </div>
                        
                        <div v-if="form.errors.photo" class="mt-1 text-sm text-red-600">
                            {{ form.errors.photo }}
                        </div>
                    </div>

                    <!-- Checkboxes -->
                    <div class="flex flex-col gap-3">
                        <label class="flex items-center">
                            <input
                                v-model="form.is_featured"
                                type="checkbox"
                                class="w-4 h-4 text-growth-600 border-gray-300 rounded focus:ring-growth-600"
                            />
                            <span class="ml-2 text-sm font-medium text-gray-700">Featured on Homepage</span>
                        </label>

                        <label class="flex items-center">
                            <input
                                v-model="form.is_approved"
                                type="checkbox"
                                class="w-4 h-4 text-growth-600 border-gray-300 rounded focus:ring-growth-600"
                            />
                            <span class="ml-2 text-sm font-medium text-gray-700">Approved</span>
                        </label>
                    </div>

                    <!-- Submit Buttons -->
                    <div class="flex gap-4 pt-4">
                        <button
                            type="submit"
                            :disabled="form.processing"
                            class="px-6 py-2 bg-growth-600 hover:bg-growth-700 text-white font-medium rounded-2xl transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                            {{ form.processing ? 'Updating...' : 'Update Testimonial' }}
                        </button>
                        <Link
                            :href="route('admin.testimonials.index')"
                            class="px-6 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium rounded-2xl transition-colors"
                        >
                            Cancel
                        </Link>
                    </div>
                </form>
            </div>
        </div>
    </AdminLayout>
</template>

<script setup>
import { ref } from 'vue';
import { useForm, Link } from '@inertiajs/vue3';
import AdminLayout from '@/Layouts/AdminLayout.vue';

const props = defineProps({
    testimonial: Object,
});

const imagePreview = ref(null);

const form = useForm({
    author_name: props.testimonial.author_name,
    author_name_bn: props.testimonial.author_name_bn,
    position: props.testimonial.position,
    position_bn: props.testimonial.position_bn,
    company: props.testimonial.company,
    company_bn: props.testimonial.company_bn,
    content: props.testimonial.content,
    content_bn: props.testimonial.content_bn,
    rating: props.testimonial.rating,
    photo: null,
    location: props.testimonial.location,
    is_featured: props.testimonial.is_featured,
    is_approved: props.testimonial.is_approved,
    _method: 'PUT',
});

const handleFileChange = (e) => {
    const file = e.target.files[0];
    if (file) {
        form.photo = file;
        const reader = new FileReader();
        reader.onload = (e) => {
            imagePreview.value = e.target.result;
        };
        reader.readAsDataURL(file);
    }
};

const submit = () => {
    form.post(route('admin.testimonials.update', props.testimonial.id));
};
</script>
