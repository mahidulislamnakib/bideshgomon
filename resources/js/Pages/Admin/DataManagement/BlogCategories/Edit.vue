<template>
  ...
  <AdminLayout>
    <div class="p-6">
      <div class="mb-6">
        <h1 class="text-2xl font-semibold text-gray-900">Edit Blog Category</h1>
        <p class="mt-1 text-sm text-gray-600">Update category information</p>
      </div>

      <div class="bg-white rounded-lg shadow p-6 max-w-2xl">
        <div v-if="blogCategory.posts_count > 0" class="mb-4 bg-red-50 border border-red-200 rounded-md p-3">
          <p class="text-sm text-brand-red-600">This category has <strong>{{ blogCategory.posts_count }}</strong> blog post(s).</p>
        </div>
        ...
        <form @submit.prevent="submit">
          ...
          <div class="mb-4">
            ...
            <input v-model="form.icon" type="text" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-brand-red-600 focus:border-brand-red-600"/>
          ...
          <div class="mb-4">
            ...
            <div class="flex gap-2">
              ...
              <input v-model="form.color" type="text" class="flex-1 px-3 py-2 border border-gray-300 rounded-md focus:ring-brand-red-600 focus:border-brand-red-600"/>
            </div>
          ...
          <div class="mb-4">
            ...
            <input v-model.number="form.order" type="number" min="0" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-brand-red-600 focus:border-brand-red-600"/>
...
const form = useForm({
  ...
  icon: props.blogCategory.icon,
  description: props.blogCategory.description,
  color: props.blogCategory.color,
  order: props.blogCategory.order,
  is_active: props.blogCategory.is_active,
});

const submit = () => {
  form.put(route('admin.data.blog-categories.update', props.blogCategory.id));
};
</script>
