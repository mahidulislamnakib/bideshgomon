<template>
  ...
  <AdminLayout>
    <div class="p-6">
      <div class="mb-6">
        <h1 class="text-2xl font-semibold text-gray-900">Edit Document Type</h1>
        <p class="mt-1 text-sm text-gray-600">Update document type information</p>
      </div>
        ...
        <form @submit.prevent="submit">
          ...
          <div class="flex justify-end gap-3">
            ...
            <button type="submit" :disabled="form.processing" class="px-4 py-2 bg-brand-red-600 text-white rounded-md hover:bg-red-700 disabled:opacity-50">
              {{ form.processing ? 'Updating...' : 'Update Document Type' }}
            </button>
          </div>
        </form>
      </div>
...
const form = useForm({
  ...
  name_bn: props.documentType.name_bn,
  slug: props.documentType.slug,
  description: props.documentType.description,
  category: props.documentType.category,
  is_required: props.documentType.is_required,
  sort_order: props.documentType.sort_order,
  is_active: props.documentType.is_active,
});

const submit = () => {
  form.put(route('admin.data.document-types.update', props.documentType.id));
};
</script>
