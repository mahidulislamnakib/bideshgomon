<template>
    <AdminLayout>
        <Head title="Language Tests Management" />
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="mb-6">
                    <div class="flex items-center justify-between">
                        <div>
                            <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Language Tests Management</h1>
                            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Manage language proficiency tests in the system</p>
                        </div>
                        <div class="flex items-center space-x-3">
                            <Link
                                :href="route('admin.data.language-tests.bulk-upload')"
                            ...
                            <Link
                                :href="route('admin.data.language-tests.create')"
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 mb-6">
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input
                                v-model="form.search"
                                type="text"
                                placeholder="Search by name, code, language..."
                    ...
                    <div class="flex items-center justify-between mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                        ...
                        <a
                            :href="route('admin.data.language-tests.export', form)"
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm overflow-hidden">
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead class="bg-gray-50 dark:bg-gray-700">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Score Type</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Status</th>
                                    <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Actions</th>
                                </tr>
                            </thead>
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                <tr v-for="test in languageTests.data" :key="test.id" class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div>
                                            <div class="text-sm font-medium text-gray-900 dark:text-white">{{ test.name }}</div>
                                            <div v-if="test.name_bn" class="text-sm text-gray-500 dark:text-gray-400">{{ test.name_bn }}</div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span v-if="test.language" class="text-sm text-gray-900 dark:text-white">{{ test.language.name }}</span>
                                        <span v-else class="text-sm text-gray-400 dark:text-gray-500">—</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="px-2 py-1 bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-white text-xs font-mono rounded">{{ test.code }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                                        <span v-if="test.min_score !== null && test.max_score !== null">{{ test.min_score }} - {{ test.max_score }}</span>
                                        <span v-else class="text-gray-400 dark:text-gray-500">—</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="px-2 py-1 bg-red-100 dark:bg-blue-900 text-brand-red-600 dark:text-blue-300 text-xs rounded capitalize">{{ test.score_type }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <button
                                            @click="toggleStatus(test)"
                                            :class="test.is_active ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300'"
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center justify-end space-x-2">
                                            <Link
                                                :href="route('admin.data.language-tests.edit', test.id)"
                    ...
                    <div class="bg-white dark:bg-gray-800 px-4 py-3 border-t border-gray-200 dark:border-gray-700 sm:px-6">
                        <Pagination :links="languageTests.links" />
                    </div>
                </div>
            </div>
        </div>
...
const filter = () => {
    router.get(route('admin.data.language-tests.index'), form, {
        preserveState: true,
        preserveScroll: true,
    });
};
...
const sortBy = (field) => {
    ...
    router.get(route('admin.data.language-tests.index'), {
        ...form,
        sort: field,
        direction,
    }, {
...
const toggleStatus = (test) => {
    if (confirm(`Are you sure you want to ${test.is_active ? 'deactivate' : 'activate'} ${test.name}?`)) {
        router.post(route('admin.data.language-tests.toggle-status', test.id), {}, {
            preserveScroll: true,
        });
    }
};
...
const deleteTest = (test) => {
    if (confirm(`Are you sure you want to delete ${test.name}? This action cannot be undone.`)) {
        router.delete(route('admin.data.language-tests.destroy', test.id), {
            preserveScroll: true,
        });
    }
};
