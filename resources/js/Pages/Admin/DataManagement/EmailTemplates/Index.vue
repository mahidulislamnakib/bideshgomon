    ...
    <div class="p-6">
      <div class="mb-6 flex items-center justify-between">
        ...
        <div class="flex gap-3">
          ...
          <Link :href="route('admin.data.email-templates.create')" class="inline-flex items-center px-4 py-2 bg-growth-600 text-white rounded-md hover:bg-growth-700">
            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="p-4 border-b border-gray-200">
          <div class="flex flex-wrap gap-4">
            <div class="flex-1 min-w-[200px]">
              <input v-model="filters.search" @input="debouncedSearch" type="text" placeholder="Search templates..." class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-growth-600 focus:border-growth-600"/>
            </div>
            <div class="w-40">
              <select v-model="filters.category" @change="applyFilters" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-growth-600 focus:border-growth-600">
            ...
            <div class="w-32">
              <select v-model="filters.is_active" @change="applyFilters" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-growth-600 focus:border-growth-600">
            ...
            <div class="w-40">
              <select v-model="filters.sort" @change="applyFilters" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-growth-600 focus:border-growth-600">
              ...
              <tr v-for="template in templates.data" :key="template.id" class="hover:bg-gray-50">
                ...
                <td class="px-6 py-4">
                  <span class="inline-flex px-2 py-1 text-xs font-medium rounded-full" :class="{
                    'bg-red-100 text-growth-600': template.category === 'general',
                ...
                <td class="px-6 py-4">
                  <button @click="toggleStatus(template)" class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-growth-600 focus:ring-offset-2" :class="template.is_active ? 'bg-growth-600' : 'bg-gray-200'">
                ...
                <td class="px-6 py-4 text-right text-sm font-medium space-x-2">
                  <Link :href="route('admin.data.email-templates.edit', template.id)" class="text-growth-600 hover:text-red-900">Edit</Link>
...
const confirmDelete = (template) => {
  if (template.campaigns_count > 0) {
    toast.error('Cannot delete template that is used in campaigns.');
    return;
  }
