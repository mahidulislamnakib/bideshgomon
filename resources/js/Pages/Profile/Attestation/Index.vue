...
const fetchApplications = async (page = 1) => {
    loading.value = true;
    try {
        const response = await axios.get(route('api.attestation-applications.index'), {
            params: { page },
        });
...
const deleteApplication = (id) => {
    if (confirm('Are you sure you want to delete this application?')) {
        axios.delete(route('api.attestation-applications.destroy', id))
            .then(() => {
...
<template>
    <Head title="Attestation Applications" />
        ...
        <div class="py-6 sm:py-12">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="mb-6 sm:mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Attestation Applications</h1>
                        <p class="mt-2 text-sm text-gray-600">Manage your student visa applications for studying abroad</p>
                    </div>
                    <Link
                        :href="route('profile.attestation.create')"
                        class="inline-flex items-center justify-center px-4 py-2 bg-growth-600 border border-transparent rounded-lg font-semibold text-white hover:bg-growth-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-growth-600 transition-colors w-full sm:w-auto"
                    >
                        ...
                        <div class="p-4 sm:p-6">
                            <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                                ...
                                <div class="flex sm:flex-col gap-2 sm:ml-4">
                                    <Link
                                        :href="route('profile.attestation.show', application.id)"
                                        class="inline-flex items-center justify-center px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-growth-600 transition-colors"
                                    >
                                        View Details
                                    </Link>
                                    <button
                ...
                <div v-else class="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
                    ...
                    <div class="mt-6">
                        <Link
                            :href="route('profile.attestation.create')"
                            class="inline-flex items-center px-4 py-2 bg-growth-600 border border-transparent rounded-lg font-semibold text-white hover:bg-growth-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-growth-600 transition-colors"
                        >
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                            </svg>
                            Create Attestation Application
                        </Link>
                    </div>
                </div>
            </div>
