    ...
    <div class="space-y-6">
        ...
        <div class="flex items-center justify-between pb-4 border-b border-gray-100">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h3 class="text-lg font-semibold text-gray-900">Visa History</h3>
                    <p class="text-sm text-gray-500">Track applications, approvals & rejections · <span class="text-red-600 font-medium">Critical for visa applications</span></p>
                </div>
            </div>
            <button
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <tr v-for="visa in visaHistory" :key="visa.id" class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <button @click="editVisaHistory(visa)" class="text-growth-600 hover:text-red-900 mr-3">
                        ...
                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-96 overflow-y-auto">
                                ...
                                <div>
                                    ...
                                    <select v-model="form.country" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-growth-600 focus:ring-growth-600">
                                ...
                                <div>
                                    ...
                                    <select v-model="form.visa_type" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-growth-600 focus:ring-growth-600">
                                ...
                                <div>
                                    ...
                                    <input v-model="form.visa_category" type="text" placeholder="e.g., B1/B2, Tier 4, Subclass 500" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-growth-600 focus:ring-growth-600" />
                                ...
                                <div>
                                    ...
                                    <input v-model="form.visa_number" type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-growth-600 focus:ring-growth-600" />
                                </div>

                                <!-- Application Date -->
                                <div>
                                    <label class="block text-sm font-medium text-gray-700">Application Date *</label>
                                    <input v-model="form.application_date" type="date" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-growth-600 focus:ring-growth-600" />
                                ...
                                <div class="col-span-2">
                                    ...
                                    <select v-model="form.status" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-growth-600 focus:ring-growth-600">
                                ...
                                <div v-if="form.status === 'approved'">
                                    ...
                                    <input v-model="form.issue_date" type="date" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-growth-600 focus:ring-growth-600" />
                                </div>

                                <!-- Expiry Date (if approved) -->
                                <div v-if="form.status === 'approved'">
                                    <label class="block text-sm font-medium text-gray-700">Expiry Date</label>
                                    <input v-model="form.expiry_date" type="date" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-growth-600 focus:ring-growth-600" />
                                </div>

                                <!-- Valid From -->
                                <div v-if="form.status === 'approved'">
                                    <label class="block text-sm font-medium text-gray-700">Valid From</label>
                                    <input v-model="form.valid_from" type="date" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-growth-600 focus:ring-growth-600" />
                                </div>

                                <!-- Valid To -->
                                <div v-if="form.status === 'approved'">
                                    <label class="block text-sm font-medium text-gray-700">Valid To</label>
                                    <input v-model="form.valid_to" type="date" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-growth-600 focus:ring-growth-600" />
                                </div>
                                ...
                                <div v-if="form.status === 'approved'">
                                    ...
                                    <input v-model="form.entry_date" type="date" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-growth-600 focus:ring-growth-600" />
                                </div>

                                <div v-if="form.status === 'approved'">
                                    <label class="block text-sm font-medium text-gray-700">Exit Date (Actual)</label>
                                    <input v-model="form.exit_date" type="date" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-growth-600 focus:ring-growth-600" />
                                </div>

                                <!-- Duration of Stay (if approved) -->
                                <div v-if="form.status === 'approved'">
                                    <label class="block text-sm font-medium text-gray-700">Duration of Stay (Days)</label>
                                    <input v-model.number="form.duration_of_stay" type="number" min="1" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-growth-600 focus:ring-growth-600" />
                                </div>

                                <!-- Purpose of Visit -->
                                <div>
                                    <label class="block text-sm font-medium text-gray-700">Purpose of Visit</label>
                                    <textarea v-model="form.purpose_of_visit" rows="2" placeholder="Describe the purpose of your visit" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-growth-600 focus:ring-growth-600"></textarea>
                                </div>
                                ...
                                <div>
                                    ...
                                    <input v-model="form.embassy_location" type="text" placeholder="e.g., US Embassy Dhaka" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-growth-600 focus:ring-growth-600" />
                                </div>

                                <!-- Visa Fee -->
                                <div>
                                    <label class="block text-sm font-medium text-gray-700">Visa Fee Paid (৳)</label>
                                    <input v-model.number="form.visa_fee" type="number" min="0" step="0.01" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-growth-600 focus:ring-growth-600" />
                                </div>
        ...
        <div v-if="showDeleteModal" class="fixed z-10 inset-0 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                        <button type="button" @click="deleteVisaHistory" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-growth-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:ml-3 sm:w-auto sm:text-sm">
                            Delete
                        </button>
